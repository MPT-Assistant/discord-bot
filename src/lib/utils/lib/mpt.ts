...
export default class MPT {
	...
	public async getLastDump(): Promise<MPT_Data> {
		const data = await DB.api.models.dump.findOne({});
		if (data) {
			this.data = data.data;
			return this.data;
		} else {
	...
	public generateKeyboard(
		...
		const buttonsRows: MessageActionRow[] = [];

		buttonsRows.push(
			new MessageActionRow().addComponents([
				new MessageButton()
					.setCustomId(command + "_" + getNextSelectDay("понедельник"))
					.setLabel("ПН")
					.setStyle("SECONDARY"),
				new MessageButton()
					.setCustomId(command + "_" + getNextSelectDay("вторник"))
					.setLabel("ВТ")
					.setStyle("SECONDARY"),
				new MessageButton()
					.setCustomId(command + "_" + getNextSelectDay("среда"))
					.setLabel("СР")
					.setStyle("SECONDARY"),
			]),
			new MessageActionRow().addComponents([
				new MessageButton()
					.setCustomId(command + "_" + getNextSelectDay("четверг"))
					.setLabel("ЧТ")
					.setStyle("SECONDARY"),
				new MessageButton()
					.setCustomId(command + "_" + getNextSelectDay("пятница"))
					.setLabel("ПТ")
					.setStyle("SECONDARY"),
				new MessageButton()
					.setCustomId(command + "_" + getNextSelectDay("суббота"))
					.setLabel("СБ")
					.setStyle("SECONDARY"),
			]),
			new MessageActionRow().addComponents([
